...
export default function Home() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setIsLoading(true);
    const isUrlSource = offerPreview?.extractionStatus === "success" && offerPreview.sourceType === "url";
    const formData = new FormData(); formData.append("jobDescription", jobDescription); formData.append("cv", cvFile); formData.append("sourceType", isUrlSource ? "url" : "manual"); if (isUrlSource && sourceUrl) formData.append("sourceUrl", sourceUrl); if (isUrlSource && offerPreview?.sourceName) formData.append("sourceName", offerPreview.sourceName);
    try {
  ...
  async function handleExtractOffer() {
    ...
    try {
      const response = await fetch("/api/extract-offer", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: sourceUrl.trim() }) });
      const data = await readJsonResponse(response);
      if (!response.ok) throw new Error(typeof data.error === "string" ? data.error : "La URL no es válida.");
      setOfferPreview(data as unknown as JobOfferPreview);
      if (data.extractionStatus !== "success") setError("No pudimos leer automáticamente este aviso. Pegá el texto del aviso y continuamos con el análisis.");
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "No pudimos leer automáticamente este aviso. Pegá el texto del aviso y continuamos con el análisis."); } finally { setIsExtracting(false); }
  }
...
function ContactCta() {
  const whatsappMessage = encodeURIComponent("Hola Carla, utilicé Tu Match Profesional y quisiera consultar por el servicio de asesoramiento de empleabilidad.");
